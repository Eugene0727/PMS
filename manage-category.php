...
<style>
 ...
 /* .isOccupied, .isReserved , .getSlotId{
...
.isOccupied,.isReserved, .getSlotId:hover {background-color: #3e8e41}
                    ...
                    <div class="card">
                        <div class="card-header">
                            <strong class="card-title">Manage Category</strong>
                              <input type="text" id="myInput" onkeyup="myFunction()" placeholder="Search Category Name" title="Type in a name">
                        </div>
                        <div class="card-body">
...
$sql = mysqli_query($con,"select * FROM tblcategory WHERE areaId ='$id'");
/*$sql2 = mysqli_query($con,"select * FROM tblparkingslot WHERE areaId ='$id' AND (status='Reserved' OR status='Occupied')");*/
$cnt=1;
$status = "";
while ($row=mysqli_fetch_array($sql)) {
    $categoryId = $row['categoryId'];
    $sql2 = mysqli_query($con,"select * FROM tblparkingslot WHERE areaId ='$id' AND categoryId='$categoryId'");
    $result = mysqli_fetch_array($sql2);
?>
                <tr>
                  <td><?php echo $cnt;?></td>
                        <td><?php echo $row['VehicleCat'];?></td>
                        <?php if($result['status'] == 'Occupied' && $result['categoryId'] == $row['categoryId']){ ?>
                          <td><button class="isOccupied">Edit Details</button></td> 
                        <?php }else if($result['status'] == 'Reserved' && $result['categoryId'] == $row['categoryId']){ ?>
                           <td><button class="isReserved">Edit Details</button></td>
                        <?php }else{ ?> 
                        <td><button class="getSlotId" onclick="getSlotId('<?php $categoryId = $row["categoryId"]; echo $categoryId ?>')">Edit Details</button></td>
                      <?php } ?>
                </tr>
                <?php 
$cnt=$cnt+1;
